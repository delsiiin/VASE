...
class LlamaAttention(nn.Module):
    ...
    def __init__(self, config: LlamaConfig, attn_hid_dim):
        super().__init__()
        self.config = config
        self.hidden_size = attn_hid_dim
...
class LlamaDecoderLayer(nn.Module):
    def __init__(self, config: LlamaConfig, layer_idx: int, attn_hid_dim):
        super().__init__()
        self.hidden_size = attn_hid_dim
        self.self_attn = (
            LlamaAttention(config=config, attn_hid_dim=attn_hid_dim)
            if not getattr(config, "_flash_attn_2_enabled", False)
            else LlamaFlashAttention2(config=config, attn_hid_dim=attn_hid_dim)
        )
        self.input_layernorm = LlamaRMSNorm(attn_hid_dim, eps=config.rms_norm_eps)
        self.layer_idx = layer_idx

    # Modified forward method to support Draft&Verify
    def forward(
        self,
        hidden_states: torch.Tensor,
        attention_mask: Optional[torch.Tensor] = None,
...
class LlamaPreTrainedModel(PreTrainedModel):
    ...
    def _set_gradient_checkpointing(self, module, value=False):
        if isinstance(module, RouterModel):
...
class RouterModel(LlamaPreTrainedModel):
    """
    ...
    def forward(
        ...
        if not return_dict:
            return tuple(v for v in [all_draft_hidden_states, next_cache, all_hidden_states, all_self_attns] if v is not None)
